...
跳过 Planner，直接调用 HealthKit 工具同步采集。日界使用 `dayOffsets(inclusiveDays: 7)`（含今天），7 天数据表中今天一行依赖真机 HealthKit 实际采样。
...
dev_flow.sh selftest
  │
  ├─ Build + Install → Launch app with --self-test
  ├─ App 内 SelfTestRunner 串行跑 16 场景（Planner 真实 API → HealthKit → Executor）
  ├─ 每个场景将结构化日志写入沙盒 Documents/selftest-N.log
  └─ 轮询拉回日志 → 生成 report.md + crash logs
```

> 注：`project.pbxproj` 无 `TrainingUITests` target，自测不走 xcodebuild test，走 launch argument 直接驱动 `SelfTestRunner`。
...
| 10 | 昨天比赛表现 | 赛后数据分析 |
| 11 | 今天状态 | today=true 查询 |
| 12 | 今天0点至今 | 时段窗口查询 |
| 13 | 查昨天 RHR | days_ago=1 单日查询 |
| 14 | 查前天 RHR | days_ago=2 单日查询 |
| 15 | 7天 RHR 表 | range=7 table 输出 |
| 16 | 赛后恢复 | 赛后 1h 恢复窗口查询 |
